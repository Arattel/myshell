...
void execute_pipe(std::string& input_line, std::set<std::string>& global,  std::set<std::string>& local,
    ...
    piped_execution(commands_with_splitted_arguments, global, local, cur_path, dup(STDIN_FILENO),
...
void piped_execution(std::vector<std::vector<std::string>>& all_piped_commands, std::set<std::string>& global,
        std::set<std::string>& local, std::string& cur_path, int saved_stdin, int saved_stdout, int saved_stderr){
    if(all_piped_commands.size() >= 2){
        ...
        if(pid == -1){
            ...
            execute_command(all_piped_commands[0], global, local, cur_path);
            ...
            piped_execution(next_execution, global, local, cur_path, saved_stdin, saved_stdout, saved_stderr);
